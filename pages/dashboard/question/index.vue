<template>
  <div class="lg:pr-[70px] py-[50px] lg:ml-[320px] xl:ml-[365px] px-4 lg:pl-0">
    ...
    <section class="pt-[50px]">
      ...
      <div v-else class="w-full px-6 py-4 my-4 bg-white rounded-lg shadow-lg"
        v-for="question in questions.data.result.data">
        <h2 class="mb-4 text-xl font-medium text-dark">Question number {{ question.id }}</h2>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50"
      ...
      <div class="w-full max-w-md mx-auto overflow-hidden bg-white rounded-lg">
        ...
        <div class="p-4">
          <form @submit.prevent="editQuestion(detailQuestion.id)">
            <div class="mb-4">
              <label class="block mb-2 font-medium text-gray-700" for="username">
                Description
              </label>
              <input
                ...
                type="text" v-model="detailQuestion.description">
            </div>
            <div class="mb-4">
              <label class="block mb-2 font-medium text-gray-700" for="choice_1">
                Choice 1
              </label>
              <input
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded appearance-none focus:outline-none focus:shadow-outline"
                type="text" v-model="detailQuestion.choice_1">
            </div>
            <div class="mb-4">
              <label class="block mb-2 font-medium text-gray-700" for="choice_2">
                Choice 2
              </label>
              <input
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded appearance-none focus:outline-none focus:shadow-outline"
                type="text" v-model="detailQuestion.choice_2">
            </div>
            <div class="mb-4">
              <label class="block mb-2 font-medium text-gray-700" for="choice_3">
                Choice 3
              </label>
              <input
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded appearance-none focus:outline-none focus:shadow-outline"
                type="text" v-model="detailQuestion.choice_3">
            </div>
...
export default {
  ...
  methods: {
    ...
    async editQuestion(id) {
      try {
        let response = await this.$axios.post(`/question/update/${id}`, this.detailQuestion)
